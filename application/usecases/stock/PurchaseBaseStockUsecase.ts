...
export class PurchaseBaseStockUsecase {
  ...
  > {
    ...
    // -----------
    // Create operations
    const [maybeCreatedToBankOperation, maybeCreatedPurchaseFeeOperation] =
      await Promise.all([
    ...
    // -----------
    // Create stock order
    const maybeCreatedStockOrder = await this.stockOrderRepository.create(
      maybeNewStockOrder
    );
    if (
      maybeCreatedStockOrder instanceof AccountNotFoundError ||
      maybeCreatedStockOrder instanceof UserNotFoundError ||
      maybeCreatedStockOrder instanceof StockNotFoundError
    ) {
      return maybeCreatedStockOrder;
    }

    // -----------
    // Create financial security
    const maybeCreatedFinancialSecurity =
      await this.financialSecurityRepository.create(maybeFinancialSecurity);
    if (
  ...
  > {
    ...
    if (maybeStock.disabled) {
      return new DisabledStockError(
        "The stock is disabled and cannot be purchased."
      );
    }

    const existingBaseStocks =
      await this.financialSecurityRepository.findAllByStock(stockId);
    if (existingBaseStocks.length >= maybeStock.baseQuantity) {
      return new InsufficientBaseQuantityError(
        "No more base stocks available for this stock."
      );
    }
